...
static void app_event_process_request_chunks_file_handler(
    void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
  ...
  do
  {
    ...
    GOTO_CHECK(read(file, &chunk_context_size, CONNECTION_REQUEST_SIZE_INFO_LENGTH) != CONNECTION_REQUEST_SIZE_INFO_LENGTH,
               TAG, "Failed to read request size", error_free_context);

    if (chunk_context_size > CONTEXT_BUFFER_MAX_LENGTH)
    {
      GOTO_CHECK(lseek(file, chunk_context_size, SEEK_CUR) == -1, TAG, "Failed to skip request content", error_close_file);
...
static void app_event_process_request_chunk_handler(
    void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
  request_chunk_data_t *chunk = (request_chunk_data_t *)event_data;
  ...
  if (chunk->processed == chunk->total)
  {
    ...
    esp_event_post(APP_EVENTS, APP_EVENT_PROCESS_REQUEST_CHUNKS_FILE, chunk, sizeof(request_chunk_data_t), portMAX_DELAY);
  }
...
void app_main(void)
{
  ...
  ESP_ERROR_CHECK(esp_event_handler_instance_register(APP_EVENTS, APP_EVENT_PROCESS_REQUEST_CHUNK, app_event_process_request_chunk_handler, NULL, NULL));
  ESP_ERROR_CHECK(esp_event_handler_instance_register(APP_EVENTS, APP_EVENT_PROCESS_REQUEST_CHUNKS_FILE, app_event_process_request_chunks_file_handler, NULL, NULL));
