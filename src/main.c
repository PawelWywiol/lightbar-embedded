                                                          ...
                                                          void *event_data)
{
    ...
    do
    {
        ...
        GOTO_CHECK(read(file, &chunk_type_info, CONNECTION_REQUEST_TYPE_INFO_LENGTH) !=
        ...
        if (chunk_context_size > CONTEXT_BUFFER_MAX_LENGTH)
        {
            GOTO_CHECK(lseek(file, chunk_context_size, SEEK_CUR) == -1, TAG, "Failed to skip request content",
                       error_close_file);
            GOTO_CHECK(read(file, &chunk_end_info, CONNECTION_REQUEST_EOL_INFO_LENGTH) !=
                           CONNECTION_REQUEST_EOL_INFO_LENGTH,
                       TAG, "Failed to read request EOL", error_close_file);
            GOTO_CHECK(chunk_end_info != CONNECTION_REQUEST_EOL_INFO, TAG, "Failed to read request EOL",
                       error_close_file);
            continue;
        }
