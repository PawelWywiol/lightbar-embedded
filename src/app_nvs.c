...
esp_err_t nvs_read_data(const char *key, void *data, size_t length)
{
  ...
  nvs_handle_t handle;
  esp_err_t err =
    nvs_open_from_partition(CONFIG_APP_NVS_PARTITION_LABEL, CONFIG_APP_NVS_NAMESPACE, NVS_READONLY, &handle);
...
esp_err_t nvs_write_data(const char *key, void *data, size_t length)
{
  ...
  nvs_handle_t handle;
  esp_err_t err =
    nvs_open_from_partition(CONFIG_APP_NVS_PARTITION_LABEL, CONFIG_APP_NVS_NAMESPACE, NVS_READWRITE, &handle);
