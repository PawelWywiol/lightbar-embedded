...
static esp_err_t set_content_type_from_file(httpd_req_t *req,
  ...
  if (CHECK_FILE_EXTENSION(filepath, ".html") ||
      CHECK_FILE_EXTENSION(filepath, ".html.gz")) {
    type = "text/html";
  } else if (CHECK_FILE_EXTENSION(filepath, ".js") ||
             CHECK_FILE_EXTENSION(filepath, ".js.gz")) {
    type = "application/javascript";
  } else if (CHECK_FILE_EXTENSION(filepath, ".css") ||
             CHECK_FILE_EXTENSION(filepath, ".css.gz")) {
    type = "text/css";
  } else if (CHECK_FILE_EXTENSION(filepath, ".png") ||
             CHECK_FILE_EXTENSION(filepath, ".png.gz")) {
    type = "image/png";
  } else if (CHECK_FILE_EXTENSION(filepath, ".jpg") ||
             CHECK_FILE_EXTENSION(filepath, ".jpg.gz")) {
    type = "image/jpeg";
  } else if (CHECK_FILE_EXTENSION(filepath, ".ico") ||
             CHECK_FILE_EXTENSION(filepath, ".ico.gz")) {
    type = "image/x-icon";
  } else if (CHECK_FILE_EXTENSION(filepath, ".svg") ||
             CHECK_FILE_EXTENSION(filepath, ".svg.gz")) {
    type = "image/svg+xml";
  } else {
    return ESP_FAIL;
  }
...
static request_network_type_t get_request_network_type(httpd_req_t *req) {
  ...
  uint32_t request_network_mask =
      (unsigned int)(inet_addr(ipstr_colon + 1) & mask);
...
static esp_err_t common_get_handler(httpd_req_t *req) {
  ...
  snprintf(filepath, FILE_SYSTEM_PATH_MAX_LENGTH, "%s%s",
           clean_vfs_path(filepath), GZIP_EXTENSION);
...
static esp_err_t api_post_handler(httpd_req_t *req) {
  ...
  request_chunk_data_t chunk = {.data = req->user_ctx,
                                .total = content_length,
                                .size = 0,
                                .processed = 0};

  snprintf(chunk.uid, UID_MAX_LENGTH, "%08x",
  ...
  do {
    ...
    esp_event_post(APP_EVENTS, APP_EVENT_PROCESS_REQUEST_CHUNK, &chunk,
...
esp_err_t init_server(app_config_t *app_config) {
  ...
  GOTO_CHECK(context == NULL, TAG,
             "Failed to allocate memory for server context", error);

  httpd_handle_t server = NULL;
  httpd_config_t config = HTTPD_DEFAULT_CONFIG();
  config.uri_match_fn = httpd_uri_match_wildcard;
  GOTO_CHECK(httpd_start(&server, &config), TAG, "Failed to start server",
             error_free_context);

  httpd_uri_t api_get_uri = {.uri = "/api",
                             .method = HTTP_GET,
                             .handler = api_get_handler,
                             .user_ctx = context};
  httpd_register_uri_handler(server, &api_get_uri);

  httpd_uri_t api_post_uri = {.uri = "/api",
                              .method = HTTP_POST,
