...
esp_err_t write_wifi_credentials(const wifi_credentials_t *wifi_credentials) {
  ...
  return nvs_write_data(CONFIG_APP_NVS_WIFI_CREDENTIALS_KEY,
                        (void *)wifi_credentials, sizeof(wifi_credentials_t));
}

esp_err_t read_wifi_credentials(wifi_credentials_t *wifi_credentials) {
  ESP_LOGI(TAG, "Reading WiFi credentials");

  if (nvs_read_data(CONFIG_APP_NVS_WIFI_CREDENTIALS_KEY,
                    (void *)wifi_credentials,
                    sizeof(wifi_credentials_t)) != ESP_OK) {
...
void reset_ap_credentials(wifi_credentials_t *ap_credentials) {
  ...
  strncpy(ap_credentials->ssid, CONFIG_APP_AP_SSID_PREFIX,
          strlen(CONFIG_APP_AP_SSID_PREFIX));
  ap_credentials->ssid[strlen(CONFIG_APP_AP_SSID_PREFIX)] = '-';
  ap_credentials->ssid[AP_SSID_MAX_LENGTH] = '\0';

  strncpy(ap_credentials->password, CONFIG_APP_AP_PASSWORD,
          strlen(CONFIG_APP_AP_PASSWORD));
...
esp_err_t write_ap_credentials(const wifi_credentials_t *ap_credentials) {
  ...
  return nvs_write_data(CONFIG_APP_NVS_AP_CREDENTIALS_KEY,
                        (void *)ap_credentials, sizeof(wifi_credentials_t));
}

esp_err_t read_ap_credentials(wifi_credentials_t *ap_credentials) {
  ESP_LOGI(TAG, "Reading AP credentials");

  if (nvs_read_data(CONFIG_APP_NVS_AP_CREDENTIALS_KEY, (void *)ap_credentials,
                    sizeof(wifi_credentials_t)) != ESP_OK) {
